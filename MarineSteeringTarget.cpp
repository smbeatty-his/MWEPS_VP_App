...
MarineSteeringTarget::MarineSteeringTarget(void) : m_pkBowWaveAngled(NULL), m_pkSternWake(NULL), m_pkHullWake(NULL),
...
MarineSteeringTarget::~MarineSteeringTarget()
{	
	if (GetClone())
	{
...
int MarineSteeringTarget::Configure(void)
{
	return vsgu::SUCCESS;
}

void MarineSteeringTarget::Update(const double dTime)
{ 
	SteeringTarget::Update(dTime);
}

void MarineSteeringTarget::PopulateTarget(const char* sTargetName)
{
	ASSERT(sTargetName)
...
void MarineSteeringTarget::InstanceTarget(ITarget* pkTarget, const char* sInstanceName)
{
	ASSERT(pkTarget)
	ASSERT(dynamic_cast<MarineSteeringTarget*>(pkTarget))
	ASSERT(sInstanceName)

	MarineSteeringTarget* pkMarineTarget = (MarineSteeringTarget*) pkTarget;
	...
	SteeringTarget::InstanceTarget(pkTarget, sInstanceName);
}

void MarineSteeringTarget::CloneShip(vpMarineShip* pkShip)
{
	ASSERT(m_pkMarineShip)
	ASSERT(pkShip)
...
void MarineSteeringTarget::CloneBowWaveAngled(vpMarineFxBowWaveAngled* pkBowWaveAngled)
{
	ASSERT(m_pkBowWaveAngled)
...
void MarineSteeringTarget::CloneSternWake(vpMarineFxSternWake* pkSternWake)
{
	ASSERT(m_pkSternWake)
	ASSERT(pkSternWake)
...
void MarineSteeringTarget::CloneHullWake(vpMarineFxHullWake* pkHullWake)
{
	ASSERT(m_pkHullWake)
	ASSERT(pkHullWake)
...
void MarineSteeringTarget::CloneMotionStrategy(vpMarineShipMotionStrategy* pkMotionStrategy)
{
	ASSERT(m_pkMotionStrategy)
